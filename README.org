...
- add server into source/config.py, change user name in run_00_start.py, add passwd in /dev/shm/p
...
- cd /dev/shm;  ssh -p 2228 localhost -l root -i key_from_here_to_google
